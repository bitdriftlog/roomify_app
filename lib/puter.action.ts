...
export const createProject = async ({item, visibility = 'private'}: CreateProjectParams): Promise<DesignItem | null | undefined> => {
    if(!PUTER_WORKER_URL) {
        console.warn('Puter worker URL not configured. Skipping project fetch.')
        return null;
    }

    const projectId = item.id;
    const hosting = await getOrCreateHostingConfig();
    const hostingSource = projectId ? await uploadImageToHosting({hosting, url: item.sourceImage, projectId, label: "source"}) : null;
    ...
    const resolvedSource = hostingSource?.url || (isHostedUrl(item.sourceImage) ? item.sourceImage : '');
    if(!resolvedSource) {
